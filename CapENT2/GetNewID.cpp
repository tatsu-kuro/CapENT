...
CGetNewID::CGetNewID(CWnd* pParent /*=NULL*/)
	: CDialogEx(CGetNewID::IDD, pParent)
{

	m_record_setid_find = 0;
}
...
void CGetNewID::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_Control(pDX, IDC_STARTCAP, m_start);
...
//	ON_BN_CLICKED(IDC_STARTCAP, &CGetNewID::OnBnClickedStartcap)
	...
	ON_BN_CLICKED(IDC_FIND, &CGetNewID::OnBnClickedFind)
END_MESSAGE_MAP()
...
//{
//	m_IDnum+=100000000;//100000000を加えて抜ける。値をチェックしてIDOKからstartcapとcurplayに分岐する
//	OnOK();
...
//	m_startcap.MoveWindow(nx+3*dx/2+1,ny+4*dy,3*nw/2,nh,1);
//	m_ok.MoveWindow(nx,ny+4*dy,3*nw/2,nh,1);



	if (m_record_setid_find == 0) {//レコード
		...
		m_cancel.MoveWindow(nx,ny+4*dy,nw,nh,1);
		m_find.MoveWindow(nx+dx,ny+4*dy,nw,nh,1);
		m_ok.MoveWindow(nx+2*dx,ny+4*dy,nw,nh,1);//setid
...
void CGetNewID::OnPaint()
{
	...
	DispNum(m_IDnum=0);//現行のIDはリセットしたが使いやすそう
...
void CGetNewID::OnBnClickedCancel()//cancel
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	CDialogEx::OnCancel();
}

void CGetNewID::OnBnClickedOk()//setid
{
	m_record_setid_find = 2;//setid 4picsも書き換える
	CDialogEx::OnOK();
}

void CGetNewID::OnBnClickedFind()//find
{
	m_record_setid_find = 3;//find 4picsは書き換えない。10picsのみ書き換える
	CDialogEx::OnOK();
}

void CGetNewID::OnBnClickedStartcap()//record start
{
	m_record_setid_find = 1;//record;
	CDialogEx::OnOK();
}